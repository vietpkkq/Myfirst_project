% Note: run by using "Run and Advance"
...
base_dir = 'G:\Giang Day\ThietKeHTTGMT\KalmanAndTracking\ExampleKF\kf_tracking_student_dave\hexbug_frames';
cd(base_dir);
...
u = 0.01; % define acceleration magnitude 0.005
...
HexAccel_noise_mag = 0.5; % 0.001; 0.1; 5.0 %process noise: the variability in how fast the Hexbug is speeding up (stdv of acceleration: meters/sec^2)
tkn_x = 0.5;  %measurement noise in the horizontal direction (x axis).
tkn_y = 0.5;  %measurement noise in the horizonta l direction (y axis).
Ez = [tkn_x 0; 0 tkn_y]; % Ma tran R
...
B = [(dt^2/2); (dt^2/2); dt; dt]; % Thanh phan gay nhieu la gia toc
C = [1 0 0 0;...
    0 1 0 0];  %this is our measurement function C, that we apply to the state estimate Q to get our expect next/new measurement
