function mykalmanfilter_tracking()
%
...
%   imshow(Im)
  ...
  PP = A*P*A' + Q; 
  % Prediction
  K = PP*H'*inv(H*PP*H'+R); % Compute the Kalman gain
  x(i,:) = (xp + K*([cc(i),cr(i)]' - H*xp))'; % Update x
  [cc(i),cr(i)];
  P = (eye(4)-K*H)*PP; % Update P
  hold on
    for c = -1*radius: radius/20 : 1*radius
      r = sqrt(radius^2-c^2);
